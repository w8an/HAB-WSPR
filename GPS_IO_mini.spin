...
CON
  ...
  CR = 13                                               ' ASCII <CR>
  LF = 10                                               ' ASCII <LF>
  serRX   = 19                                          ' Serial RX pin
  serTX   = 20            ' Serial TX pin
...
PUB start | okay, cog    
  ...
  okay := uart.start(serRX, serTX, %0000, 9600)     'rxPin, txPin, mode, baud    uart.init(serRX, serTX, 9600)
   
  if okay > 0
    cog := cognew(readNEMA, @gps_stack)
    ''// todo: push airborne mode setting into gps device
  return cog
...
PUB readNEMA
  Null[0] := 0
  repeat
    longfill(gps_buff,20,0)      'clear buffer
